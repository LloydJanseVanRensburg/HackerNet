...
exports.createNewPollPage = catchAsync(async (req, res, next) => {
...
exports.createNewPoll = catchAsync(async (req, res, next) => {
  let { forum, title, question, ans1, ans2, ans3 } = req.body;
  let user_id = req.user.user_id;

  // Create a poll
  const poll = new Poll(user_id, forum, title);
  const [resA, _a] = await poll.save();
  let poll_id = resA.insertId;

  // Add questions to poll
  const poll_question = new PollQuestion(poll_id, 1, question);
  const [resB, _b] = await poll_question.save();
  let question_id = resB.insertId;
...
exports.pollPage = catchAsync(async (req, res, next) => {
...
exports.createPollVote = catchAsync(async (req, res, next) => {
  let { answer } = req.body;
...
exports.updatePollPage = catchAsync(async (req, res, next) => {
  let poll_id = req.params.id;
  let user_id = req.user.user_id;
